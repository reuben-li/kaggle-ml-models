...
def load_data_sparse(data_path=DATA_PATH):
    ...
    train_df["price_t"] = (train_df["price"]) /(train_df["bathrooms"] + train_df["bedrooms"] + 1.0)
    test_df["price_t"] =  (test_df["price"])/ (test_df["bathrooms"] + test_df["bedrooms"] +  1.0)
    ...
    train_df["weekend"] = train_df["created_day"].apply(lambda x: weekend(x))
    test_df["weekend"] = test_df["created_day"].apply(lambda x: weekend(x))
    ...
    #le = LabelEncoder()
    #le.fit(train_df["yearmonth"].append(test_df["yearmonth"]))
    #train_df["yearmonth"] = le.transform(train_df["yearmonth"]) 
    #test_df["yearmonth"] = le.transform(test_df["yearmonth"]) 
    ...
    train_df["exp_price"] = pd.DataFrame(clf.predict(train_df[xparams]).tolist()).set_index(train_df.index)
    train_df["overprice"] = train_df["price"] - train_df["exp_price"]
    
    test_df["exp_price"] = pd.DataFrame(clf.predict(test_df[xparams]).tolist()).set_index(test_df.index)
    test_df["overprice"] = test_df["price"] - test_df["exp_price"]

    categorical = [ "grid","display_address", "manager_id", "building_id","street_address","num_furniture","halfbr","toobig","address_distance","nophoto","weekend"]
...
#            categorical.append(categorical[f] + "_" +categorical[s])
    ...
    features_to_use.extend(["overprice","num_photos", "num_features", "num_description_words", "created_month", "created_day", "listing_id", "created_hour","total_days","diff_rank",#"listing_rank","total_days_rank",
    "price_t","price_latitue","price_longtitude"])#,"price_latitue_longtitude"]) "created_year", #,"num_description_length"
