...
prods['reorders'] = priors['reordered'] \
    .groupby(priors.product_id).sum().astype(np.float32)
...
#    return order_group.shape[0], order_group.order_id[ix], \
#    order_group.add_to_cart_order.mean()
...
userXproduct.last_order_id = \
    userXproduct.last_order_id.map(lambda x: x[1]).astype(np.int32)
...
def features(selected_orders, labels_given=False):
    ...
    for row in selected_orders.itertuples():
        ...
        if labels_given:
            labels += [(order_id, product) in train.index
    ...
    df['user_average_days_between_orders'] = \
        df.user_id.map(users.average_days_between_orders)
    ...
    df['days_since_prior_order'] = \
        df.order_id.map(orders.days_since_prior_order)
    df['days_since_ratio'] = \
    ...
    df['UP_last_order_id'] = \
        df.z.map(userXproduct.last_order_id)
